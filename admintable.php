    ...
    <table class="table table-bordered table-hover">
        ...
        <tbody>
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                if ($variables[0] === 'Modyfikuj') {
                    echo "<td class='text-center'> 
                    <button data-bs-toggle='modal' data-bs-target='#exampleModal' onclick='changeIdFormValue({$id})' class='{$variables[2]}' value={$id}>{$variables[0]}</button>
                    </td>";
                } else {
                    echo "<td class='text-center'> <form action='{$variables[3]}' method='post'>
                    <button type='submit' class='{$variables[2]}' name='idnumber' value={$id} >{$variables[0]}</button>
                     </form></td>";
                }
                echo "</tr>";
                $i++;
